...
function tableMaximizeSize( pTable, pSize ) {
    if (pTable.rows.length <= pSize) {
        return;
    }
    for( var  i = pTable.rows.length - 1; i > pSize; i--) {
        pTable.deleteRow(i);
    }
    return null;
}
...
function tableInsert( pTable, pTabDef, jObject, atTop )
{
   var tRow = undefined;
   var tKeys = pTabDef.keys;

    if (atTop) {
       tRow = pTable.insertRow(1);
    } else {
       pTable.insertRow( pTable.rows.length);
    }
